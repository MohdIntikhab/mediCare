...
import { Calendar, Grid, Home, List, Menu,X, PlusSquare, UserPlus, Users } from 'lucide-react';
...
// Sliding Active Indicator 
  ...
  //It will help on scrolling on x-axis 
  useEffect(() => {
    const container = navInnerRef.current;
    if (!container) return;
  ...
  // when user signed in , fetch a token and store in a local storage 
  useEffect(()=>{
    let mounted = true;
    const storeToken = async () =>{
      ...
      try{
        ...
        console.warn("Could not retrieve the clerk Token", err);
      }
    } 
    storeToken();
    return ()=>{
  ...
  const handleSignOut = async()=>{
    ...
    try{
      ...
      console.error("Signed Out Failed", err);
    } 
    finally{
     try{
       localStorage.removeItem("clerk_token");

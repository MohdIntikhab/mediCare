...
const AddService = ({serviceId}) => { 
  ...
  // Date/Time control for adding the slot 
  const now = new Date();
  ...
  //intital value as Strings matching options 
  ...
  //user cant select past year/month/date from here 
  useEffect(() => {
  ...
  // to fetch services when in editing stage 
  ...
  function handleImageChange(e) {
    ...
    //Remove the existing image if the user found the new image 
    setRemoveImage(false);
    setHasExistingImage(false);
  }
  ...
  // to prevent the user for selecting the past time for that particular date 
  ...
  // to validate that all fields are filled by users or not  
  function validate() {
    const newErrors = {};
       ...
       <form onSubmit={handleSubmit} className={addServiceStyles.container.form}>
        ...
        <div>
          ...
          <p className={addServiceStyles.header.subtitle}>
            Create a Beutiful Service Card with unique  time slots 
          </p>
        </div>

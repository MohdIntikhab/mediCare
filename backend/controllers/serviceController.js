...
// so this function convert the array like input into a clean array 
//when empty or invalid , it return a clean array i.e: [];
...
// so this function take date-time slot strings and group them into a 
...
export async function getServices(req,res){
  try{
    ...
    const list = await Service.find().sort({createdAt: -1}).lean(); // “.lean() Mongoose document ko simple JavaScript object me convert karta hai for better performance”
    ...
    const {id} = req.params; 
     ...
     if(existing.imagePublicId){
      try{
        ...
        console.warn("Failed to delete image from Cloudinarey:", 
          err?.message || err
        );
      }
     } 

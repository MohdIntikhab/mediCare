...
//to getServiceAppointmentyBy id 
export const getServiceAppointmentById = async(req , res)=>{
  ...
  try{
    ...
    return res.json({success: true, date: appt});
  } 
   catch (err) {
...
export const  updateServiceAppointment = async(req, res)=>{
  try{
    ...
    //first check whether fill if yess then update the feild 
    ...
    const updated = await ServiceAppointment.findByIdAndUpdate(id, {$set: updates},{ //$seT se sirf whi changes honge jo updates mai present hai
...
export const getServiceAppointmentStats = async(req, res) => {
  ...
  try{
    ...
    console.error("getServicesAppointment Stats error:", err);
    return res.status(500).json({ success: false, message: "Server error" });
  } 
}
...
export const getServiceAppointmentsByPatient = async(req, res)=>{
  try{
    ...
    console.error("getServicesAppointment By Patient error :", err);
    return res.status(500).json({ success: false, message: "Server error" });
  } 
}

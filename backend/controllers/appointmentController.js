...
//this function will create a frontend url 
const buildFrontendBase = (req) => {
  if (FRONTEND_URL) return FRONTEND_URL.replace(/\/$/, "");
  const origin = req.get("origin") || req.get("referer"); //req.get() = header read karna origin->Direct heaader read karta hai Origin: http://localhost:5173
  if (origin) return origin.replace(/\/$/, "");
  const host = req.get("host"); //header se host fetch karta hai ex- host: 4000
...
export const getAppointmentsByPatient = async(req, res)=>{
  try{
    ...
    console.error("Get AppointmentByPatient  Error:", err);
    return res.status(500).json({
      success: false,
      message: "Server Error"
    })
...
export const createAppointment = async(req, res)=>{
  try{
    ...
    // Ye URLs hain jaha user redirect hoga after payment
...
export const confirmPayment = async(req, res)=>{
  try{ 
    ...
    let session;
    try{ 
      // Ye Stripe ka ek module (object) hai jo checkout/payment sessions ko handle karta hai
...
export const getAppointmentsByDoctor = async(req,res)=>{
  try{
    ...
    console.error("getAppointments By Doctor   Error:", err);
    return res.status(500).json({ success: false, message: "Server error" });
  } 
} 
